          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {roadmapStages.map((stage) => (
              <Card className="shadow-none" key={stage.title}>
                ...
                <h2 className="mt-3 font-serif text-2xl text-ink">{stage.title}</h2>
        ...
        <Container className="space-y-10">
          ...
          <div className="flex flex-wrap gap-3">
            {futurePage.stageLanguage.map((label) => (
              <span className="rounded-[8px] border border-ink/10 bg-paper px-3 py-2 text-xs font-medium uppercase text-ink" key={label}>
                {label}
              </span>
            ))}
          </div>
