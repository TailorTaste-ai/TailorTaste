...
const variantClasses: Record<CTA["variant"], string> = {
  ...
  secondary: "border border-ink/20 bg-chalk/75 text-ink hover:-translate-y-0.5 hover:border-accent/60 hover:bg-chalk dark:border-chalk/20 dark:bg-chalk/88 dark:text-ink dark:hover:bg-chalk",
  inverted: "border border-chalk bg-chalk text-ink hover:-translate-y-0.5 hover:bg-paper",
  text: "min-h-0 px-0 py-0 text-ink underline underline-offset-4 hover:text-accent dark:text-chalk dark:hover:text-accent",
};
...
export function Button({ label, href, variant, className = "" }: ButtonProps) {
  return (
    <Link
      className={`inline-flex min-h-11 items-center justify-center rounded-full px-5 py-3 text-sm font-semibold leading-none transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35 focus-visible:ring-offset-2 focus-visible:ring-offset-paper dark:focus-visible:ring-offset-ink ${variantClasses[variant]} ${className}`}
      href={href}
    >
      {label}
    </Link>
